...
def get_read_evtx(files_found, report_folder, seeker, wrap_text):
    print("-------------------------------------------------------- Reading File ------------------------------------------------------------------")
    reading = False
    event_list = []
    ...
    for filename in files_found:
        with Evtx(filename) as evtx:
            ...
            if len(event_list) > 0:
                report = ArtifactHtmlReport('Passkey Logs')
                report.start_artifact_report(report_folder, 'Passkey Logs')
                report.add_script()
                data_headers = ('userId', 'transaction_id', 'type', 'browser', 'browserPath', 'website', 'timestamp', 'computerName','device', 'result')

                report.write_artifact_data_table(data_headers, event_list, filename)
                report.end_artifact_report()

                tsvname = f'Passkey Logs'
                tsv(report_folder, data_headers, event_list, tsvname)
            else:
                logfunc('Passkey Logs data available')
